...
dat.pop.t <- read_xlsx("data/Population_Altitude.xlsx", sheet="Sheet1") %>%
    mutate(Year = as.factor(Year)) %>%
    group_by(Year) %>%
    summarise(Pop_total = sum(Population)) %>%
    ungroup()
  
dat.pop <- read_xlsx("data/Population_Altitude.xlsx", sheet="Sheet1") %>%
    mutate(Year = as.factor(Year)) %>%
    full_join(dat.pop.t) %>%
...
data.obs.exp.altitude <- data_age90 %>%
    ...
    select(Year, Altitude, Observed, Expected, Population, Pop_total, Number, Perc)
