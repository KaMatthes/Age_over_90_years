function_altitude_sex_diff <- function(Name) {
  
  load(paste0("../data/data.obs.exp.sex.altitude.", Name, ".RData"))

  data.model <- data.obs.exp.sex.altitude
    
    mod_sum <- summary(Model_alt_sex <- glm(Observed ~ Sex , family="poisson",offset=log(Population), data=data.model ))
