...
dat.pop <- read_xlsx("data/Language_district_1888_1900.xlsx") %>%
    group_by(Language, Year) %>%
    summarize(Population=sum(Population)) %>%
    ungroup() %>%
    mutate(Year = as.factor(Year))
  
dat.pop.t <- read_xlsx("data/Language_district_1888_1900.xlsx") %>%
    group_by(Year) %>%
...
data.expected <- data_age90 %>%
  ...
  data.obs.exp.language <- data_age90 %>%
    select(Wohnkanton, Erhebungsjahr) %>%
    mutate(
      Language = Wohnkanton,
    ...
    select(Year, Language, Observed, Expected, Population, Pop_total, Number, Perc)
