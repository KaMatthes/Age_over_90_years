...
dat.pop.t <- read_xlsx("data/Population_urbanity.xlsx", sheet="Sheet1") %>%
    mutate(Year = as.factor(Year)) %>%
    group_by(Year) %>%
    summarise(Pop_total = sum(Population)) %>%
    ungroup()

dat.pop <- read_xlsx("data/Population_urbanity.xlsx", sheet="Sheet1") %>%
    mutate(Year = as.factor(Year)) %>%
    full_join(dat.pop.t) %>%
...
data.obs.exp.urbanity <- dat.pop  %>%
    ...
    select(Year, Urbanity, Observed, Expected, Population, Pop_total, Number, Perc)
